...
class DataConfig:
  ...
  batch_size           = batch_size_per_replica * dst.num_replicas_in_sync
...
class SupconConfig:
  ...
  class training:
    ...
    callbacks = lambda: [
      ...
      ReduceLRBacktrack(best_path=SupconConfig.training.weights, patience=10, min_lr=1e-6, distributed_strategy=dst),
    ]
  ...
  class finetune:
    ...
    callbacks = lambda: [
      tf.keras.callbacks.TerminateOnNaN(),
      tf.keras.callbacks.EarlyStopping(patience=50, verbose=1),
      tf.keras.callbacks.TensorBoard(SupconConfig.finetune.logs, histogram_freq=1, write_graph=True, profile_batch=0),
      tf.keras.callbacks.ModelCheckpoint(SupconConfig.finetune.weights, save_weights_only=True, save_best_only=True, verbose=1),
      ReduceLRBacktrack(best_path=SupconConfig.finetune.weights, patience=10, min_lr=1e-6, distributed_strategy=dst),
    ]
